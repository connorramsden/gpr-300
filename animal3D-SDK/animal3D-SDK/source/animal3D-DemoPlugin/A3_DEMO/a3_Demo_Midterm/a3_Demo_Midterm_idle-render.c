...
/*
	...
	a3_DemoMidterm_idle-render.c
	Demo mode implementations: midterm rendering.
...
// controls for midterm mode
void a3midterm_render_controls(a3_DemoState const* demoState, a3_Demo_Midterm const* demoMode,
	a3f32 const textAlign, a3f32 const textDepth, a3f32 const textOffsetDelta, a3f32 textOffset)
{
	// display mode info
	a3byte const* pipelineText[midterm_pipeline_max] = {
		"Shading",
		"Shading with MRT",
	};
	a3byte const* targetText_back[midterm_target_back_max] = {
		"Color buffer",
	};
	a3byte const* targetText_fbo[midterm_target_fbo_max] = {
	...
	a3byte const* const* targetText[midterm_pipeline_max] = {
		targetText_back,
		targetText_fbo,
	};
	...
	a3byte const* displayProgramName[midterm_display_max] = {
		"Texture",
		"Color manipulation",
	...
	// pipeline and target
	a3_Demo_Midterm_RenderProgramName const render = demoMode->render;
	a3_Demo_Midterm_DisplayProgramName const display = demoMode->display;
	a3_Demo_Midterm_PipelineName const pipeline = demoMode->pipeline;
	a3_Demo_Midterm_TargetName const targetIndex = demoMode->targetIndex[pipeline];
	a3_Demo_Midterm_TargetName const targetCount = demoMode->targetCount[pipeline];

	// demo modes
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Pipeline (%u / %u) ('[' | ']'): %s", pipeline + 1, midterm_pipeline_max, pipelineText[pipeline]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"        Target (%u / %u) ('{' | '}'): %s", targetIndex + 1, targetCount, targetText[pipeline][targetIndex]);

	// lighting modes
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Rendering mode (%u / %u) ('j' | 'k'): %s", render + 1, midterm_render_max, renderProgramName[render]);

	// specific modes
	if (pipeline == midterm_fbo)
		a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
			"    Display mode (%u / %u) ('J' | 'K'): %s", display + 1, midterm_display_max, displayProgramName[display]);
}


//-----------------------------------------------------------------------------

// sub-routine for rendering the demo state using the midterm pipeline
void a3midterm_render(a3_DemoState const* demoState, a3_Demo_Midterm const* demoMode)
{
	...
	const a3_DemoStateShaderProgram* displayProgram[midterm_pipeline_max][midterm_display_max] = {
		{
			0,
		}, {
			demoState->prog_drawTexture,
	...
	const a3_Framebuffer* writeFBO[midterm_pipeline_max] = {
		0,
	...
	a3_Demo_Midterm_DisplayProgramName const display = demoMode->display;
	a3_Demo_Midterm_PipelineName const pipeline = demoMode->pipeline;
	a3_Demo_Midterm_TargetName const targetIndex = demoMode->targetIndex[pipeline], targetCount = demoMode->targetCount[pipeline];
